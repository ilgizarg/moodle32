...
class file_info_context_course extends file_info {
	 ...
	 * @return file_info|null file_info instance or null if not found or access not allowed
	 */
	protected function get_area_course_instruction($itemid, $filepath, $filename) {
		global $CFG;
		...
		$filepath = is_null($filepath) ? '/' : $filepath;
		$filename = is_null($filename) ? '.' : $filename;
		if (!$storedfile = $fs->get_file($this->context->id, 'course', 'instriction', 0, $filepath, $filename)) {
			if ($filepath === '/' and $filename === '.') {
				$storedfile = new virtual_root_file($this->context->id, 'course', 'instruction', 0);
			} else {
				// not found
				return null;
			}
		...
		$urlbase = $CFG->wwwroot . '/pluginfile.php';
		return new file_info_stored($this->browser, $this->context, $storedfile, $urlbase, get_string('areacourseinstruction', 'repository'), false, true, true, false);
	}
	 ...
	 * @return file_info|null file_info instance or null if not found or access not allowed
	 */
	protected function get_area_course_workprogrammfiles($itemid, $filepath, $filename) {
		global $CFG;
		...
		$filepath = is_null($filepath) ? '/' : $filepath;
		$filename = is_null($filename) ? '.' : $filename;
		if (!$storedfile = $fs->get_file($this->context->id, 'course', 'workprogrammfiles', 0, $filepath, $filename)) {
			if ($filepath === '/' and $filename === '.') {
				$storedfile = new virtual_root_file($this->context->id, 'course', 'workprogrammfiles', 0);
			} else {
				// not found
				return null;
			}
		...
		$urlbase = $CFG->wwwroot . '/pluginfile.php';
		return new file_info_stored($this->browser, $this->context, $storedfile, $urlbase, get_string('areacourseworkprogrammfilesfiles', 'repository'), false, true, true, false);
	}
	...
	private function get_filtered_children($extensions = '*', $countonly = false, $returnemptyfolders = false) {
		$areas = array(
			...
			array('course', 'instruction'),
			array('course', 'workprogramfiles'),
